...
describe("#compile", function () {
  ...
  it("reports errors on bad source", function (done) {
    ...
    var compileProcess = compiler.compile(prependFixturesDir("Bad.nim"), opts);

    compileProcess.on("exit", function (exitCode) {
      var desc = "Expected nim make to have exit code 1";
  ...
  it("throws when given an unrecognized argument", function () {
    ...
    expect(function () {
      var compileProcess = compiler.compile(prependFixturesDir("Parent.nim"), opts);
...
describe("#compileToString", function () {
  ...
  it("adds runtime options as arguments", function () {
    ...
    return expect(compiler
      ._prepareProcessArgs("a.nim", opts)
      .join(" ")).to.equal("make a.nim +RTS -A128M -H128M -n8m -RTS");
  });
  ...
  it("reports errors on bad syntax", function () {
    ...
    var compilePromise = compiler.compileToString(prependFixturesDir("Bad.nim"), opts);
  ...
  it("reports type errors", function () {
    ...
    var compilePromise = compiler.compileToString(prependFixturesDir("TypeError.nim"), opts);
  ...
  it("Rejects the Promise when given an unrecognized argument like `yes`", function () {
    ...
    var compilePromise = compiler.compileToString(prependFixturesDir("Parent.nim"), opts);
  ...
  it("works when run multiple times", function () {
    ...
    var runCompile = function () {
      ...
      return compilePromise.then(function (result) {
        var desc = "Expected nim make to return the result of the compilation";
        expect(result.toString(), desc).to.be.a('string');
      });
    };

    // Compiling in parallel leads to issues with the cache. Therefore we chain
    // the compilations instead. For details, see https://github.com/nim/compiler/issues/1853.
    // This issue is tracked as https://github.com/rtfeldman/node-nim-compiler/issues/86.
  ...
  it("handles output suffix correctly", function () {
    ...
    return compiler.compileToString(prependFixturesDir("Parent.nim"), opts)
...
describe("#compileWorker", function () {
  ...
  it("works with BasicWorker.nim", function () {
    var opts = {
      verbose: true,
      cwd: fixturesDir
    };
    var compilePromise = compiler.compileWorker(
      prependFixturesDir(""),
      prependFixturesDir("BasicWorker.nim"),
      "BasicWorker"
    );
